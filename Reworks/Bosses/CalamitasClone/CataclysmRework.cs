...
{
    public class CataclysmRework : GlobalNPC
    {
        private static Type ModType = typeof(Cataclysm);
        ...
        private delegate string OriginalGetTexture(Cataclysm self);

        private string OnTexture(OriginalGetTexture origin, Cataclysm self)
        {
            if (self.Type == Type)
                return (GetType().Namespace + "." + "Cataclysm").Replace('.', '/');
        ...
        private int Type
        {
            get
            {
                if (type < NPCID.Count)
                {
                    type = ModContent.NPCType<Cataclysm>();
                }

                if (type < NPCID.Count) throw new Exception("Not found cataclysm type");
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            texture2D15 = Cataclysm.GlowTexture.Value;
